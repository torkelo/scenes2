# @grafana/base-ui — Agent guide

Grafana-styled shadcn / [Base UI](https://base-ui.com/) component set for the Product Design Engineering Agentic Experience Platform. shadcn components built on Base UI primitives, styled with Grafana design tokens (`Dialog`, `Select`, `Tabs`, `Tooltip`, …). Independent of `@grafana/ui` and `@grafana/data`.

## Critical rules

**Package scope: ports only** — This package holds Grafana-styled ports of the upstream shadcn / Base UI components, nothing else. A new component that builds on these primitives (a bespoke composite or an AXP-specific pattern) belongs in [`@grafana/components`](../components/README.md), or in another package if it doesn't fit there — never here. This keeps the package a pure, maximally reusable port, and avoids a name collision if upstream later ships a component we want to adopt under the same name. Precedent: `IconCallout` lives in `@grafana/components` for exactly this reason.
...
- [ ] Added to `packages/base-ui/src/index.ts`
- [ ] Storybook with `Base UI/` namespace
