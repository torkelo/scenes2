# @grafana/ai-elements — Agent guide

Grafana-styled AI Elements component set for the Product Design Engineering Agentic Experience Platform. Building blocks for AI interfaces: chat, reasoning, tool-call, code, and canvas surfaces (`Message`, `Reasoning`, `Tool`, `Conversation`, `PromptInput`, …), built on [`@grafana/base-ui`](../base-ui/README.md). Independent of `@grafana/ui` and `@grafana/data`.

## Critical rules

**Package scope: ports only** — This package holds Grafana-styled ports of the upstream [Vercel AI Elements](https://ai-sdk.dev/elements) components, nothing else. A new component that builds on these components or on `@grafana/base-ui` primitives (a bespoke composite or an AXP-specific pattern) belongs in [`@grafana/components`](../components/README.md), or in another package if it doesn't fit there — never here. This keeps the package a pure, maximally reusable port, and avoids a name collision if upstream later ships a component we want to adopt under the same name.
...
- shadcn / Base UI components from [`@grafana/base-ui`](../base-ui/README.md) — compose these rather than rebuilding equivalent primitives.
- Base UI primitives from [`@base-ui/react`](https://base-ui.com/).
- Standard React, Emotion (`@emotion/css`), and Motion (`motion/react`).
...
- [ ] Added to `packages/ai-elements/src/index.ts`
- [ ] Storybook with `AI Elements/` namespace
- [ ] Tests using `getByRole` queries
