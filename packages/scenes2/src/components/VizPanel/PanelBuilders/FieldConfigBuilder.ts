...
> implements StandardFieldConfigInterface<
  StandardFieldConfig,
  FieldConfigBuilder<TFieldConfig>,
  'set'
> {
    ...
    const fieldConfig: FieldConfigSource<TFieldConfig> = {
      defaults: {
        custom: this.defaultFieldConfig
          ? cloneDeep(this.defaultFieldConfig())
          : ({} as TFieldConfig),
  ...
  public setFieldConfigDefaults<T extends keyof StandardFieldConfig>(
    key: T,
    value: StandardFieldConfig[T],
  ) {
    this._fieldConfig.defaults = {
      ...this._fieldConfig.defaults,
      [key]: value,
